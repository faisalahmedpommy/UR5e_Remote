    ...
    private void Update()
    {
        ...
        if (TimeElapsed > 1/frequency)
        {
            ...
            stateMsg.header.stamp.sec = (int)Math.Truncate(time);
             long nowUnixMilliseconds = DateTimeOffset.Now.ToUnixTimeMilliseconds();

        // Get the start of today (midnight) in Unix time
        DateTimeOffset startOfToday = new DateTimeOffset(DateTimeOffset.Now.Year, DateTimeOffset.Now.Month, DateTimeOffset.Now.Day, 0, 0, 0, DateTimeOffset.Now.Offset);
        long startOfTodayUnixMilliseconds = startOfToday.ToUnixTimeMilliseconds();

        // Calculate the difference in milliseconds
        long millisecondsToday = nowUnixMilliseconds - startOfTodayUnixMilliseconds;

        Console.WriteLine("Total milliseconds since the start of today: " + millisecondsToday);
#else
            stateMsg.header.stamp.sec = (uint)Math.Truncate(time);
            stateMsg.header.seq++;
#endif
            stateMsg.header.stamp.nanosec = (uint)(millisecondsToday);
